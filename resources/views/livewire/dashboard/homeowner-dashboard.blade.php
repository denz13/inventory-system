            ...
            <div class="overflow-x-auto pb-4 alerts-scroll" style="scroll-behavior: smooth;">
                <div class="flex gap-4" style="min-width: min-content;">
                    ...
                    <div class="flex-shrink-0" style="min-width: 400px; max-width: 400px;">
                        <div class="alert show box bg-danger text-white flex items-start p-4 rounded-lg h-full" role="alert">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-6 h-6 mr-3 flex-shrink-0 mt-1">
                                <path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path>
                                <line x1="12" y1="9" x2="12" y2="13"></line>
                                <line x1="12" y1="17" x2="12.01" y2="17"></line>
                            </svg>
                            <div class="flex-1">
                                <div class="font-bold">URGENT:</div>
                                <div class="mt-1">Your account has an overdue balance. Please check the Billing Payment section.</div>
                            </div>
                        </div>
                    </div>
                    @endif

                    <!-- Approved Landlord Applications -->
                    ...
                    <div class="flex-shrink-0" style="min-width: 400px; max-width: 400px;">
                        <div class="alert show box bg-success text-white flex items-start p-4 rounded-lg h-full" role="alert">
                            ...
                            <div class="flex-1">
                                <div class="font-bold">Landlord Application Approved:</div>
                                <div class="mt-1">Your Landlord Application has been approved.</div>
                            </div>
                        </div>
                    </div>
                    @endforeach
        ...
        <div class="col-span-12 lg:col-span-6">
            ...
            <div class="intro-y box p-5">
                <div class="flex items-center mb-4">
                    <h2 class="text-lg font-medium">Recent Billing History</h2>
                    <a href="/billing-payment" class="ml-auto text-primary text-sm">View All</a>
                </div>
                ...
                @forelse($recentBillings as $billing)
                    @php
                        ...
                        if (strtolower($billing->status) === 'approved' || strtolower($billing->status) === 'paid') {
                            $statusColor = 'text-success';
                        } elseif (strtolower($billing->status) === 'rejected' || strtolower($billing->status) === 'declined') {
                        ...
                        if (strpos($billingDateRange, ' - ') !== false) {
                            ...
                            if (count($dateParts) >= 2) {
                                ...
                                try {
                                    ...
                                    $isOverdue = $billingEndDate->isPast() && strtolower($billing->status) !== 'approved' && strtolower($billing->status) !== 'paid';
                                } catch (\Exception $e) {
                                    $isOverdue = false;
                                }
                            }
                        ...
                        if ($isOverdue) {
                            ...
                            $statusText = ucfirst($billing->status);
                        }
                    @endphp
                    ...
                    <div class="flex items-center py-3 border-b border-slate-200">
                        <div class="flex-1">
                            <div class="font-medium">
                                @if($billing->billingItems->isNotEmpty())
                                    {{ \Carbon\Carbon::parse($billing->created_at)->format('F') }}: {{ $billing->billingItems->first()->description }}
                                @else
                                    {{ \Carbon\Carbon::parse($billing->created_at)->format('F') }}: Billing
                                @endif
                            </div>
                            <div class="text-xs text-slate-500 mt-0.5">{{ $billingDateRange }}</div>
                            <div class="text-xs text-slate-500">Billing ID: #{{ str_pad($billing->id, 6, '0', STR_PAD_LEFT) }}</div>
                        </div>
                        <div class="text-right">
                            <div class="font-medium">₱{{ number_format($billing->amount_due, 2) }}</div>
                            <div class="text-sm {{ $statusColor }} font-medium">{{ $statusText }}</div>
                        </div>
                    </div>
                @empty
                    <div class="text-center py-8 text-slate-500">
                        ...
                        <div class="font-medium">No billing history</div>
        ...
        <div class="col-span-12 lg:col-span-6">
            ...
            <div class="intro-y box p-5">
                ...
                @forelse($recentAppointments as $appointment)
                    <div class="flex items-center py-3 border-b border-slate-200">
                        <div class="flex-1">
                            <div class="font-medium">{{ $appointment->appointment_type ?? 'Appointment' }}</div>
                        ...
                        <div>
                            @php
                                ...
                                } elseif (strtolower($appointment->status) === 'rejected' || strtolower($appointment->status) === 'declined') {
